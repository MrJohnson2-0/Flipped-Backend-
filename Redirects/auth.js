...
app.post("/account/api/oauth/token", async (req, res) => {
    ...
    switch (req.body.grant_type) {
        ...
        case "password":
            ...
            isClientServer = regex.test(email);
            
            
            if (isClientServer && config.fasterGameServerLoad) {
                const findUser = await User.findOne({ email: email.toLowerCase() });
                if (findUser) {
                    req.user = findUser;
                    functions.advancedLog("LOGIN", "The user with the E-MAIL " + email.toLowerCase() + " has logged in" );
                }
                else {
                    const numberWith8Digits = Math.floor(10000000 + Math.random() * 90000000);
                    const registerUser = await functions.registerUser(numberWith8Digits, `server_${email.split("@")[0]}`, email, password, true);
