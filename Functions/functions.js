...
const profileManager = require("../AccountHandling/profile.js");
...
function getItemShop() {
    const catalog = JSON.parse(fs.readFileSync(path.join(__dirname, "..", "ItemShopHandler", "itemshop.json")).toString());
    const CatalogConfig = JSON.parse(fs.readFileSync(path.join(__dirname, "..", "ItemShopHandler", "ItemShopGlobals.json").toString()));
...
async function registerUser(discordId, username, email, plainPassword, isServer) {
...
function UpdateTokens() {
    fs.writeFileSync("./TokenHandler/tokens.json", JSON.stringify({
